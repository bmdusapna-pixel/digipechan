...
// Backend schema for new QR type
...
export const newQRTypeSchema = z.object({
    ...
    qrUseCases: z.array(z.string().min(1)).min(1, "At least one use case is required"),
    ...
    qrFormatType: z.nativeEnum(qrFormatType).optional().default(qrFormatType.SQUARE),
    pdfTemplate: z.string().url().optional(),
    deliveryType: z
        .array(z.nativeEnum(DeliveryType))
        .min(1)
        .optional()
        .default([DeliveryType.ETAG, DeliveryType.PHYSICAL_SHIP, DeliveryType.BULK_GENERATION]),
