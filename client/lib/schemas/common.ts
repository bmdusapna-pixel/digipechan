...
export const objectIdSchema = z.string().regex(/^[a-f\d]{24}$/i, { message: "Invalid ObjectId format" });
...
export const vehicleNumberSchema = z.string().regex(/^[A-Z]{2}\d{2}[A-Z]{2}\d{4}$/, {
    message: "Vehicle number must be in format 'XX00XX0000' without spaces",
});

export const gstNumberSchema = z.string().regex(/^[0-3][0-9][A-Z]{5}\d{4}[A-Z][A-Z0-9]Z[A-Z0-9]$/, {
