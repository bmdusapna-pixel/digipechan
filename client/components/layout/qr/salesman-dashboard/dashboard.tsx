...
import { getAssignedBundles, getSalesmanStats, getSoldQrs } from "@/types/salesman";
import { Package, TrendingUp, Users, Eye, LoaderPinwheel } from "lucide-react";

export default function SalesmanDashboard() {
  const router = useRouter();
  const token = useAuthStore((s) => s.user?.accessToken);
  const isSales = useAuthStore((s) => s.user?.roles?.includes?.(UserRoles.SALESPERSON));
  ...
  console.log("🔍 AUTH DEBUG:", {
    ...
    hasRole: useAuthStore.getState().hasRole(UserRoles.SALESPERSON)
  });

  useEffect(() => {
    // Avoid redirecting until token is available (prevents false logouts during hydration)
    if (token && !isSales) {
  ...
  const { data: bundlesRes, isLoading: bundlesLoading, error: bundlesError } = useQuery({
  ...
  const { data: soldRes, isLoading: soldLoading, error: soldError } = useQuery({
  ...
  console.log("Dashboard Debug:", {
    ...
    isSales,
    userRoles: useAuthStore.getState().user?.roles,
    user: useAuthStore.getState().user,
    statsError,
    bundlesError,
    soldError,
    API_DOMAIN: process.env.NEXT_PUBLIC_BACKEND_BASE_URL
  });
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="text-center space-y-2">
          <h1 className="text-3xl font-bold text-gray-900 mt-10">Sales Dashboard</h1>
          <p className="text-gray-600">Manage your QR bundles and track sales</p>
        </div>
          ...
          <Card className="bg-white shadow-lg border-0">
            <CardContent className="p-6">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-600">Bundles Assigned</p>
                  <p className="text-2xl font-bold text-gray-900">
                    {statsLoading ? "..." : (stats?.inventory?.totalBundles ?? 0)}
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <Card className="bg-white shadow-lg border-0">
            <CardContent className="p-6">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-600">Available QRs</p>
                  <p className="text-2xl font-bold text-gray-900">
                    {statsLoading ? "..." : (stats?.inventory?.availableQRs ?? 0)}
                  </p>
                </div>
              </div>
            </CardContent>
        ...
        <Tabs defaultValue="bundles" className="space-y-6">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="bundles" className="text-sm font-medium">
              My Bundles
            </TabsTrigger>
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              {(bundle.qrTypeId as any)?.qrName ?? "Unknown Type"}
                            </p>
                          </div>
                          <div className="flex items-center justify-between">
                            ...
                            <Button
                              size="sm"
                              onClick={() => handleViewBundle(bundle.bundleId)}
              ...
              <CardContent className="space-y-4">
                ...
                {soldLoading ? (
                  ...
                  <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
                    {(soldRes?.qrs ?? soldRes?.data?.qrs ?? []).map((qr: any) => (
                      <div
                        key={qr._id}
