...
export const createSalesperson = expressAsyncHandler(
...
export const updateSalespersonPassword = expressAsyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      return ApiResponse(
        res,
        200,
        "Salesperson password updated successfully",
        true,
        updatedSalesperson
      );
    } catch (error) {
      ...
      return ApiResponse(
        res,
        500,
        "Failed to update salesperson password",
        false,
        null
      );
    }
...
export const updateSalesperson = expressAsyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { salespersonId } = req.params;
...
export const toggleSalespersonStatus = expressAsyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { salespersonId } = req.params;
      ...
      return ApiResponse(
        res,
        500,
        "Failed to update salesperson status",
        false,
        null
      );
    }
...
export const toggleSalespersonVerification = expressAsyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { salespersonId } = req.params;
      ...
      const updatedSalesperson = await Salesman.findByIdAndUpdate(
        salespersonId,
        { $set: { isVerified: !salesperson.isVerified } },
        { new: true, select: "-password" }
      );
      ...
      return ApiResponse(
        res,
        200,
        `Salesperson ${updatedSalesperson?.isVerified ? "verified" : "unverified"} successfully`,
        true,
        updatedSalesperson
      );
    } catch (error) {
      ...
      return ApiResponse(
        res,
        500,
        "Failed to update salesperson verification",
        false,
        null
      );
    }
