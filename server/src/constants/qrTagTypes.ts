...
export const QR_TAG_QUESTIONS: Record<QRTagType, QRTagQuestion[]> = {
  [QRTagType.CAR_TAG]: [
...
export const getQuestionsForTagType = (tagType: QRTagType): QRTagQuestion[] => {
  return QR_TAG_QUESTIONS[tagType] || [];
};

export const getTagTypeFromString = (tagTypeString: string): QRTagType | null => {
  const upperCaseString = tagTypeString.toUpperCase();
  if (Object.values(QRTagType).includes(upperCaseString as QRTagType)) {
    return upperCaseString as QRTagType;
  }
  return null;
};

export const getTagTypeDisplayName = (tagType: QRTagType): string => {
  const displayNames: Record<QRTagType, string> = {
